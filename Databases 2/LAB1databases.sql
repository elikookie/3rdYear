...
select supplier_id from b2_supplier where supplier_id not in  (SELECT DISTINCT Supplier_Id FROM B2_STOCK);

SELECT C.Customer_Name
FROM B2_CUSTOMER C
INNER JOIN B2_CORDER CO ON C.Customer_Id = CO.Customer_Id
INNER JOIN B2_CORDERLINE COL ON CO.Corderno = COL.Corderno
INNER JOIN B2_STOCK S ON COL.Stock_Code = S.Stock_Code
...
INNER JOIN B2_CORDER CO ON C.Customer_Id = CO.Customer_Id
INNER JOIN B2_CORDERLINE COL ON CO.Corderno = COL.Corderno
INNER JOIN B2_STOCK S1 ON COL.Stock_Code = S1.Stock_Code
WHERE S1.Stock_Description = 'Phillips screwdriver'
AND EXISTS (
    SELECT 1
    FROM B2_CORDERLINE COL2
    INNER JOIN B2_STOCK S2 ON COL2.Stock_Code = S2.Stock_Code
    WHERE COL2.Corderno = CO.Corderno
    AND S2.Stock_Description = 'Box of 6" screws'
);

SELECT DISTINCT STAFF.Staff_name
FROM B2_STAFF STAFF
INNER JOIN B2_SORDER SO ON STAFF.Staff_no = SO.StaffIssued
INNER JOIN B2_SORDERLINE SOL ON SO.SOrderNo = SOL.SOrderNo
INNER JOIN B2_STOCK S ON SOL.Stock_Code = S.Stock_Code
INNER JOIN B2_SUPPLIER SUP ON S.Supplier_Id = SUP.Supplier_Id
WHERE SUP.Supplier_Name = 'Paul Sloan';

SELECT S.Supplier_Id, S.Supplier_Name, SO.SOrderNo
FROM B2_SUPPLIER S
LEFT JOIN B2_SORDER SO ON S.Supplier_Id = SO.Supplier_Id
WHERE SO.DeliveredDate IS NULL;

SELECT CO.Corderno, COL.Stock_Code, (COL.QuantityRequired * S.Unit_Price) AS Cost
FROM B2_CORDER CO
INNER JOIN B2_CORDERLINE COL ON CO.Corderno = COL.Corderno
INNER JOIN B2_STOCK S ON COL.Stock_Code = S.Stock_Code;

SELECT CO.Corderno, SUM(COL.QuantityRequired * S.Unit_Price) AS TotalCost
FROM B2_CORDER CO
INNER JOIN B2_CORDERLINE COL ON CO.Corderno = COL.Corderno
INNER JOIN B2_STOCK S ON COL.Stock_Code = S.Stock_Code
GROUP BY CO.Corderno;
...
INNER JOIN B2_STOCK S ON COL.Stock_Code = S.Stock_Code
GROUP BY CO.Corderno
HAVING SUM(COL.QuantityRequired * S.Unit_Price) > 2000;
